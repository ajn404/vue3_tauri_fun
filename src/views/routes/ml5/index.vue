<template>
  <div class="view" ref="view">
    ...
    <div class="fixed w-1/2 flex right-0 top-10 items-center">
      <span>SCORE</span>
      <el-tooltip
        content="举起手来(以鼻子为基准,当前镜头最好只有一人)"
        :visible="score === 0.1"
        effect="customized"
      >
    ...
    <img
      v-show="reactiveData.surrender"
      :src="`${baseUrl}imgs/splash/white-flag.jpg`"
      class="img w-1/2 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"
      alt=""
    />
  </div>
</template>
            ...
            for (let j = 0; j < poses[i].pose.keypoints.length; j += 1) {
              ...
              if (keypoint.score > score.value) {
                ...
                if (keypoint.part.includes('Eye')) {
                  ctx.fillStyle = `rgba(${(x * 100) % 255},${(y * 100) % 255},${
                    (x * 100) % 255
                  },${x % 1})`;
