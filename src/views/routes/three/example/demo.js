...
var values = new (function () {
  ...
  this.Help = function () {
    alert(
      'Touch friendly. Two finger drag to rotate, or hold shift and use the mouse. CTRL+SHIFT+arrows = camera snap translate. Disable chrome://flags/#overscroll-history-navigation for better touch experience'
    );
  };
  this.Undo = function () {
    if (undoHistory.length > 0) {
      ...
      if (undoHistory[0] > 2 && scene.children.length > 2) undoHistory.shift();
      if (h.length > 0) redoHistory.unshift(h);
...
function get3dPointZAxis(event) {
  ...
  if (values['MousePointMode'] == true) return p;
  else return pos;
}
...
function doDraw(event) {
  ...
  if (lastPoint) {
    ...
    if (
      Math.abs(lastPoint.x - pos.x) < 500 &&
      Math.abs(lastPoint.y - pos.y) < 500 &&
      Math.abs(lastPoint.z - pos.z) < 500
    ) {
