import { nextTick, onUnmounted, watch, type Ref, onDeactivated } from 'vue';
import '../three.scss';
...
export const useMeshBasicMaterial = async (
  container: Ref<null | HTMLElement>,
  selectedMaterial: Ref<selectMaterial>
) => {
  let rid = 0;

  const setup = async (val?: selectMaterial) => {
    ...
    mesh.material = choose(gui, mesh, geometry, val || selectedMaterial.value, [
      light1,
      light2,
      light3,
    ]);
  ...
  const { beforeDestroy } = await setup();
